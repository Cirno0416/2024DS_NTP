...
//    }
    ...
    private NTP_Timestamp_Data Receive(NTP_Timestamp_Data NTP_Timestamp) {
        ...
        if (0 < RecvPacket.getLength()) {
            ...
            // Extract fractional seconds for milliseconds (bytes 44-47)
            long f1 = (long) bRecvBuf[44] & 0xFF;
            long f2 = (long) bRecvBuf[45] & 0xFF;
            long f3 = (long) bRecvBuf[46] & 0xFF;
            long f4 = (long) bRecvBuf[47] & 0xFF;
            long fractionalPart = (f1 << 24) + (f2 << 16) + (f3 << 8) + f4;
            // Convert fractional seconds to milliseconds
            long milliSeconds = (fractionalPart * 1000L) / 0x100000000L;
            ...
            NTP_Timestamp.lMilliSecond = milliSeconds;
        } else {
            NTP_Timestamp.lUnixTime = 0; // Signal that an error occurred
        }
        return NTP_Timestamp;
