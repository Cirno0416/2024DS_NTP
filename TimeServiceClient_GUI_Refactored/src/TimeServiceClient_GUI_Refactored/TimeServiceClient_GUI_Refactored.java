...
public class TimeServiceClient_GUI_Refactored extends javax.swing.JFrame implements ActionListener {
    ...
    public TimeServiceClient_GUI_Refactored() {
        // Listener for the 'URL selection changed' event
        m_SelectionListener_NTPServerURLs = listSelectionEvent -> {
    ...
    /**
     * Initial timeout occurs after 100 ms (sends first NTP request).
     * Subsequent NTP request occur at 10-second intervals.
     */
    void Start_Timer_SendNTPRequests() {
        m_Timer_SendNTPRequests = new Timer();
        m_Timer_SendNTPRequests.scheduleAtFixedRate(new Get_NTP_Timestamp(), 100, 10000);
    }

    class Get_NTP_Timestamp extends TimerTask {
        /**
         * Send NTP requests every time it runs.
         * If the request is successful, it will update the number of requests and responses, and display UNIX time and UTC time on the UI.
         * If the request fails, it will increase the number of requests but not the number of responses.
         */
        public void run() {
...
//                    jTextField_UNIX_Time.setText(Long.toString(NTP_Timestamp.lUnixTime));
                    jTextField_UNIX_Time.setText(Long.toString(NTP_Timestamp.lUnixTimeMilli)); // MilliSecond as a unit
                    String sUTC_Time = String.format("%02d:%02d:%02d.%03d", NTP_Timestamp.lHour, NTP_Timestamp.lMinute,
                            NTP_Timestamp.lSecond, NTP_Timestamp.lMilliSecond);
                    jTextField_UTC_Time.setText(sUTC_Time);
                    UpdateStatisticsDisplay();
                    break;
                case NTP_ServerAddressNotSet, NTP_SendFailed:
                    break;
    ...
    /**
     * Add multiple servers to the NTP server address and location list.
     */
    ...
    public static void main(String[] args) throws Exception {
        ...
        java.awt.EventQueue.invokeAndWait(() -> new TimeServiceClient_GUI_Refactored().setVisible(true));
    }
    ...
    private void initComponents() {
        ...
        JScrollPane_NTPServerURLs = new JScrollPane(jList_NTPServerURLs,
        ...
        JScrollPane_NTPServerLocations = new JScrollPane(jList_NTPServerLocations,
                ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
                ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
